...
<template>
  <div
    v-show="isAlertOpen"
    class="fixed inset-0 z-[200] flex h-screen w-screen flex-col p-4 backdrop-blur-sm sm:p-6 md:p-[10vh] lg:p-[12vh] bg-black/30 dark:bg-neutral-900/90">
    <div
      ...
      <div class="flex flex-col items-center gap-3 p-10">
        <div
          ...
          <ExclamationTriangleIcon class="h-14 w-14 text-red-500 dark:text-red-300" />
        </div>
        <h2
        ...
        <button
          @click="reload"
          class="rounded bg-red-500 px-3 py-2 text-white dark:bg-red-900 font-semibold">
          Перезагрузить
        </button>
      </div>
    </div>
