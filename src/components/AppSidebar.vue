...
<template>
  ...
  <div v-else>
    <Teleport to="#header">
      <div
        class="mx-4 flex items-center border-t border-zinc-900/10 py-4 dark:border-zinc-300/10 lg:hidden"
      >
        <button
          @click="openMenu"
          type="button"
        ...
        <ol class="ml-4 flex min-w-0 whitespace-nowrap text-sm leading-6">
          ...
          <li class="truncate font-semibold text-zinc-900 dark:text-zinc-200">
            {{ title }}
          </li>
        </ol>
      </div>
    ...
    <TransitionRoot :show="isMenuOpen" as="template">
      <Dialog as="div" @close="closeMenu" class="fixed inset-0 z-50 overflow-y-auto lg:hidden">
        <TransitionChild
          as="template"
        ...
        >
          <div
            class="fixed inset-0 bg-black/20 backdrop-blur-sm transition-opacity dark:bg-zinc-900/80"
          ></div>
        </TransitionChild>
        <TransitionChild
          as="template"
        ...
        >
          <DialogPanel
            class="relative z-50 min-h-screen w-80 max-w-[calc(100%-3rem)] bg-white p-8 backdrop-blur-sm dark:bg-zinc-800"
          >
            <button
              @click="closeMenu"
            ...
            <AppNav :topics="topics" :handler="closeMenu" />
          </DialogPanel>
        </TransitionChild>
      </Dialog>
    </TransitionRoot>
